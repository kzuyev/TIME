...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::time);
    ...
    if (!settings.contains("nAnonymizetimeAmount"))
        settings.setValue("nAnonymizetimeAmount", 1000);

    nAnonymizetimeAmount = settings.value("nAnonymizetimeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizetimeAmount"))
        SoftSetArg("-anonymizetimeamount", settings.value("nAnonymizetimeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in time.cpp during shotdown to also remove the window positions
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizetimeAmount:
            return QVariant(nAnonymizetimeAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizetimeAmount:
            nAnonymizetimeAmount = value.toInt();
            settings.setValue("nAnonymizetimeAmount", nAnonymizetimeAmount);
            emit anonymizetimeAmountChanged(nAnonymizetimeAmount);
            break;
        case CoinControlFeatures:
